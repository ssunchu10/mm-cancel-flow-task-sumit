...
export const getDownsellVariant = async (
  userId: string,
...
export const acceptDownsell = async (
  userId: string,
  subscriptionId: string
) => {
  try {
    const { data, error } = await supabaseAdmin
      .from("cancellations")
      .update({ accepted_downsell: true, updated_at: new Date().toISOString() })
      .eq("user_id", userId)
      .eq("subscription_id", subscriptionId)
      .select("*")
      .single();
    ...
    console.error("acceptDownsell error:", err);
    throw err;
  }
};
